		...
		return (
			<>
				<div className="get-pro">
					<img src={logo} alt={__('Logo', 'coachpress-lite')} />
					<h3>{__('CoachPress', 'coachpress-lite')}</h3>
					<p>{__('Get access to all unlimited features.', 'coachpress-lite')}</p>
					<a className="cw-button-btn primary-btn" target="_blank" href={cw_dashboard.get_pro}>{__('Get Pro', 'coachpress-lite')}</a>
				</div>
			</>
		);
	}
